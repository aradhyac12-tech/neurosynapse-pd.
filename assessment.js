// NeuroCompass-PD v4.2 - Complete Assessment System with MDS-UPDRS
// Global state with UPDRS scoring
const STATE = {
    patient: null,
    voiceTrials: 0,
    voiceMetrics: [],
...
// UPDRS Scoring System (0-4 scale)
const UPDRS_SCORING = {
    speech: [
        { min: 0, max: 0.2, score: 0, label: 'Normal' },
        { min: 0.2, max: 0.5, score: 1, label: 'Slight' },
        { min: 0.5, max: 1.0, score: 2, label: 'Mild' },
        { min: 1.0, max: 2.0, score: 3, label: 'Moderate' },
        { min: 2.0, max: 5.0, score: 4, label: 'Severe' }
    ],
    tremor: [
        { min: 0, max: 4, score: 0, label: 'None' },
        { min: 4, max: 4.5, score: 1, label: 'Slight' },
        { min: 4.5, max: 5.0, score: 2, label: 'Mild' },
        { min: 5.0, max: 5.5, score: 3, label: 'Moderate' },
        { min: 5.5, max: 10, score: 4, label: 'Severe' }
    ],
    gait: [
        { min: 0.85, max: 1.2, score: 0, label: 'Normal' },
        { min: 0.75, max: 0.85, score: 1, label: 'Slightly slow' },
        { min: 0.65, max: 0.75, score: 2, label: 'Mildly slow' },
        { min: 0.55, max: 0.65, score: 3, label: 'Moderately slow' },
        { min: 0, max: 0.55, score: 4, label: 'Severely slow' }
    ],
    facial: [
        { min: 18, max: 30, score: 0, label: 'Normal' },
        { min: 14, max: 18, score: 1, label: 'Slight decrease' },
        { min: 10, max: 14, score: 2, label: 'Mild decrease' },
        { min: 6, max: 10, score: 3, label: 'Moderate decrease' },
        { min: 0, max: 6, score: 4, label: 'Severe decrease' }
    ]
};
...
function initializePage() {
    ...
    // Speak welcome
    setTimeout(() => {
        speak('Welcome to NeuroCompass PD. MDS-UPDRS Clinical Assessment Suite.');
    }, 500);
...
// ==================== GENUINE METRICS FUNCTIONS ====================

// Voice metrics with genuine ranges
function calculateGenuineVoiceMetrics() {
    ...
    // UPDRS scoring for voice - based on jitter
    let score = 0;
...
// Tremor metrics with genuine ranges
function calculateGenuineTremorMetrics() {
    ...
    // UPDRS scoring for tremor - based on frequency
    let score = 0;
...
// Gait metrics with genuine ranges
function calculateGenuineGaitMetrics() {
    ...
    // UPDRS scoring for gait - based on speed
    let score = 0;
...
// Facial metrics with genuine ranges
function calculateGenuineFacialMetrics() {
    ...
    // UPDRS scoring for facial - based on blink rate
    let score = 0;
...
function completeVoiceTest() {
    ...
    // Calculate average UPDRS score from all trials
    ...
    STATE.radarData.voice = 1 - (avgScore / 4); // Normalize
...
function updateResults() {
    ...
    const maxUpdrs = 24; // 6 items × 4 max
