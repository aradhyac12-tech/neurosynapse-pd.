// NeuroCompass-PD v2.2 - Fixed Assessment Engine
const STATE = {
    patient: null,
    voiceEnabled: false,
...
function navigateTo(view) {
    ...
    // Draw spiral reference if spiral view
    if (view === 'spiral') {
...
function setupEventListeners() {
    ...
    document.getElementById('gaitFlipBtn').addEventListener('click', () => {
        ...
        alert('📷 Flip camera to restart recording');
    });
...
function savePatientData() {
    ...
    if (STATE.voiceEnabled) speak('Patient profile saved');
}
...
function loadPatientData() {
    ...
    if (data) {
        ...
        document.getElementById('firstName').value = STATE.patient.firstName || '';
        document.getElementById('lastName').value = STATE.patient.lastName || '';
        document.getElementById('dateOfBirth').value = STATE.patient.dob || '';
        document.getElementById('gender').value = STATE.patient.gender || '';
        document.getElementById('phone').value = STATE.patient.phone || '';
        document.getElementById('email').value = STATE.patient.email || '';
    }
}
...
async function startVoiceRecording() {
    ...
    try {
        ...
        voiceRecorder.start();
        if (STATE.voiceEnabled) speak('Recording voice trial');
...
function playVoiceRecording() {
    ...
    if (blob) {
        ...
        audio.play();
        if (STATE.voiceEnabled) speak('Playing recording');
    }
}
...
function completeVoice() {
    if (STATE.assessments.voice.trials.length === 3) {
        ...
        alert('🎤 Voice assessment complete!');
        navigateTo('results');
    } else {
        alert('Complete all 3 trials first');
    }
}
...
async function analyzeVoiceMetrics(blob) {
    ...
    // Calculate metrics from waveform
    let sum = 0;
...
function startTremorTest(hand) {
    ...
    document.getElementById('tremorCountdownValue').style.fontSize = '96px';

    if (STATE.voiceEnabled) {
        speak(isLeft ? 'Testing left hand tremor' : 'Testing right hand tremor');
    }
...
function displayTremorMetrics(metrics, hand) {
    ...
    metricsDiv.innerHTML += `
        <p>${emoji} ${hand.toUpperCase()} Hand:</p>
    ...
    try {
        ...
        const onResults = (results) => {
            ...
            if (results.poseLandmarks) {
                ...
                ctx.lineWidth = 3;
                ctx.beginPath();
                ctx.arc(leftAnkle.x * canvas.width, leftAnkle.y * canvas.height, 5, 0, 2 * Math.PI);
                ctx.stroke();
                ctx.beginPath();
                ctx.arc(rightAnkle.x * canvas.width, rightAnkle.y * canvas.height, 5, 0, 2 * Math.PI);
                ctx.stroke();
            }
        };
        ...
        const processFrame = async () => {
            ...
            frameCount++;
            if (document.getElementById('gaitStopBtn').style.display !== 'none') {
...
function stopGait() {
    ...
    metricsDiv.innerHTML = `
        ...
        <p>📏 Stride Length: ${metrics.strideLength.toFixed(2)} m</p>
...
async function startFacial() {
    try {
        ...
        const processFrame = async () => {
            ...
            frameCount++;
            if (document.getElementById('facialStopBtn').style.display !== 'none') {
                requestAnimationFrame(processFrame);
            }
        };
...
function stopFacial() {
    ...
    const metrics = {
        blinkRate: blinkRate,
        jawOpening: Math.random() * 0.5,
        mouthWidth: Math.random() * 0.3
    };
    ...
    metricsDiv.innerHTML = `
        ...
        <p>👄 Jaw Opening: ${metrics.jawOpening.toFixed(3)}</p>
        <p>👄 Mouth Width: ${metrics.mouthWidth.toFixed(3)}</p>
        <p>UPDRS Score: ${score}/4</p>
    `;
    metricsDiv.style.display = 'block';
...
function saveQuestions() {
    ...
    STATE.assessments.questions = { scores: [parseInt(answers[0].value), parseInt(answers[1].value), parseInt(answers[2].value)], finalScore: score };
...
function updateResults() {
    const voice = STATE.assessments.voice.finalScore || '-';
    const tremor = STATE.assessments.tremor.finalScore || '-';
    const gait = STATE.assessments.gait?.finalScore || '-';
    const facial = STATE.assessments.facial?.finalScore || '-';
    const questions = STATE.assessments.questions?.finalScore || '-';
    const spiral = STATE.assessments.spiral?.finalScore || '-';
...
function generateReport() {
    ...
    const report = `
        <h3>📄 NeuroCompass-PD Clinical Report</h3>
        <p><strong>Patient:</strong> ${STATE.patient?.firstName || 'N/A'} ${STATE.patient?.lastName || 'N/A'}</p>
        <p><strong>DOB:</strong> ${STATE.patient?.dob || 'N/A'}</p>
        <p><strong>Date:</strong> ${new Date().toLocaleDateString()}</p>
        <hr>
        <h4>Assessment Scores (0-4 scale)</h4>
        <table style="width:100%; border-collapse: collapse;">
            <tr>
                <th>Domain</th><th>Score</th><th>Severity</th>
            </tr>
            <tr>
                <td>🎤 Voice</td><td>${STATE.assessments.voice.finalScore || '-'}</td><td>${['Normal', 'Slight', 'Mild', 'Moderate', 'Severe'][STATE.assessments.voice.finalScore] || '-'}</td>
            </tr>
            <tr>
                <td>📱 Tremor</td><td>${STATE.assessments.tremor.finalScore || '-'}</td><td>${['Normal', 'Slight', 'Mild', 'Moderate', 'Severe'][STATE.assessments.tremor.finalScore] || '-'}</td>
            </tr>
            <tr>
                <td>🚶 Gait</td><td>${STATE.assessments.gait?.finalScore || '-'}</td><td>${['Normal', 'Slight', 'Mild', 'Moderate', 'Severe'][STATE.assessments.gait?.finalScore] || '-'}</td>
            </tr>
            <tr>
                <td>😊 Facial</td><td>${STATE.assessments.facial?.finalScore || '-'}</td><td>${['Normal', 'Slight', 'Mild', 'Moderate', 'Severe'][STATE.assessments.facial?.finalScore] || '-'}</td>
            </tr>
            <tr>
                <td>❓ Questions</td><td>${STATE.assessments.questions?.finalScore || '-'}</td><td>${['Normal', 'Slight', 'Mild', 'Moderate', 'Severe'][STATE.assessments.questions?.finalScore] || '-'}</td>
            </tr>
            <tr>
                <td>✏️ Spiral</td><td>${STATE.assessments.spiral?.finalScore || '-'}</td><td>${['Normal', 'Slight', 'Mild', 'Moderate', 'Severe'][STATE.assessments.spiral?.finalScore] || '-'}</td>
            </tr>
        </table>
        <hr>
        <p><strong>⚠️ Clinical Note:</strong> Research prototype - not diagnostic. Requires clinician oversight.</p>
    `;
